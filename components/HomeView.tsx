...
export const HomeView: React.FC<HomeViewProps> = ({ onOpenSearch, onOpenBudget }) => {
  ...
  useEffect(() => {
    if (settings.budget.enabled) {
        setTimeRange(settings.budget.displayType);
    }
          ...
          <div className="mb-4">
            ...
            <div className="flex justify-center bg-gray-200/50 dark:bg-zinc-800/50 p-0.5 rounded-lg">
                 {(['week', 'month', 'year'] as const).map(range => (
                     <button key={range} onClick={() => setTimeRange(range)} className={`flex-1 py-1 text-xs font-medium rounded-md transition-all ${timeRange === range ? 'bg-white dark:bg-zinc-700 shadow-sm text-ios-text' : 'text-ios-subtext'}`}>{range === 'week' ? '周' : range === 'month' ? '月' : '年'}</button>
                 ))}
            </div>
          </div>
