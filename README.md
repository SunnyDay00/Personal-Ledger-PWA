...
When the user scrolls away from the latest message, the conversation view shows a floating down button that returns directly to the bottom. The composer is a real flex footer instead of an overlay, so scrolling older messages never places the input box over message content. Browser/PWA keyboard positioning coalesces `visualViewport` changes, while the Capacitor build—configured with `KeyboardResize.None`—uses only native keyboard-height events and does not replay iOS viewport animation frames through React layout. Completed Markdown message bubbles are memoized, and the bottom navigation animates only compositor-friendly transform/opacity properties. The bottom navigation is hidden only while a keyboard is actually visible and is restored when it closes.

After the AI view has been opened once, switching to Home, Ledgers, or Settings keeps that view mounted in the background. An in-flight DeepSeek request therefore continues, saves its result locally, and is visible when the user returns. Explicitly switching the long-press Statistics/AI mode away from AI still closes the AI view.
