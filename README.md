...
> 当前版本：7.4.2。本版本重点修复账目本地持久化可靠性、离线待同步队列、旧 IndexedDB 迁移保护，并清理既有构建警告。
...
- 下载成功后，会优先从 GitHub 远端项目的 `package.json` 读取版本号命名文件，例如 `7.4.2` 会保存为 `builds/ios/7.4.2.ipa`；如果读取失败，则回退为 `builds/ios/run-<运行ID>.ipa`
