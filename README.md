...
When the user scrolls away from the latest message, the conversation view shows a floating down button that returns directly to the bottom. The composer is a real flex footer instead of an overlay, so scrolling older messages never places the input box over message content; while the keyboard is closed, it sits just above the floating bottom navigation with only a small gap. Browser/PWA keyboard positioning coalesces `visualViewport` changes, while the Capacitor build—configured with `KeyboardResize.None`—uses only native keyboard-height events and does not replay iOS viewport animation frames through React layout. Completed Markdown message bubbles are memoized, and the bottom navigation animates only compositor-friendly transform/opacity properties. The bottom navigation is hidden only while a keyboard is actually visible and is restored when it closes.

After the AI view has been opened once, switching to Home, Ledgers, or Settings keeps that view mounted in the background. An in-flight DeepSeek request therefore continues, saves its result locally, and is visible when the user returns. The hidden composer releases its focus before the tab changes, preventing iOS from retaining a stale focused textarea that can no longer reopen the system keyboard. Explicitly switching the long-press Statistics/AI mode away from AI still closes the AI view.

An empty conversation shows four tappable example questions and a **Refresh** action. Refresh asks the selected DeepSeek model for four different supported read-only question ideas, sending only the previously displayed example text and generation instructions—not ledger records or metadata. This small request consumes provider tokens. If the device is offline or the request fails, the app immediately replaces the examples from a built-in local question pool. While a response is starting, one of several stable, human-friendly thinking messages is selected for that turn and displayed in a compact content-sized bubble instead of always showing a fixed database-query phrase in a full-width card.
...
1. 前端先判断本轮是否涉及账本数据。普通交流、能力询问或对回答的反馈不携带查询工具，由 DeepSeek 正常对话；明确的数据问题、“收入呢”“为什么这么高”等数据追问，以及对上一轮查询建议回复“要”“好”“可以”“继续”时，都会继承最近的数据查询意图并携带只读工具定义。
...
8. 最终文字在显示和保存前仍有一层轻量本地兜底校验，但只拦截明确事实冲突，例如未成功查询就回答账本数据、用聚合结果编造单笔明细、明细表出现不存在的分类或备注、零匹配却声称找到了记录。分类、分类组和趋势表中的占比或解释性文字不会再被当成虚构明细。明确冲突会要求模型只修正有问题的数据部分；一次重答仍失败时，前端生成的保底答案也会保留工具返回的分类、时间或账本分组，而不是退化成只有总数的固定模板。
9. 历史助手的完整文字和精简查询轨迹会保留在上下文中，以便理解语气、连续追问和“要/继续”等确认回复。原始工具结果不会跨轮重复上传；旧金额及结论会被标记为历史内容，后续引用相关数据时会重新查询当前账本。这样延续的是对话意图和查询范围，而不是把可能已经过时的数字当作永久事实。回答中的“查询依据”会显示使用的工具、账本、实际日期范围、筛选条件、记录数以及是否发生截断。
