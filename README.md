...
> 当前版本：7.6.4。本版本优化自动记录管理：列表改为带圆角和留白的面板，新增紧凑汇总与批量开启 / 关闭，新建规则默认开启，并直接显示金额、计划和启用状态。
...
- 买卖本使用单一类目，录入时选择买入 / 卖出；买入和卖出都会先选择买卖本类目，买入增加类目库存，卖出再从有剩余的买入批次中分配卖出数量
...
- 底部添加按钮点击或长按都会先展开两个入口，并带有弹性上浮动画：记账本为添加支出 / 添加收入，买卖本为添加买入 / 添加卖出；选择入口后再进入分类选择和账目录入
...
- 金额小键盘支持加减乘除表达式输入，长按减号可切换为 ÷，长按加号可切换为 ×，录入页可一键收起或展开底部小键盘
- 自定义分类与分类分组
- 分类管理仅作用于当前账本；需要管理其他账本时先切换到账本本身
- 分类管理支持稳定拖动排序，拖动提示位于卡片下方，拖动过程中列表不抖动，分类顺序调整会直接写回本地数据
- 设置 > 管理 > 自动记录可创建普通记账本的收入 / 支出模板；自动记录管理页使用带圆角和留白的列表面板，顶部紧凑汇总全部数量、开启数量、总支出和总收入，点击“全部”可批量开启或关闭自动记录，列表直接显示名称、账本、分类、金额、计划时间和启用状态；新建时在带圆角的近全屏面板中先填写名称、账本和图标，再配置类型、分类、金额和每日 / 每周 / 每月固定时间，新建保存后默认开启；自动记录不限制为 4 个，入口仅显示启用数和总数；App 打开或回到前台时会补齐错过的周期，同一期账目使用稳定 ID 去重，登录同一账号的多设备不会重复生成
- 周 / 月 / 年统计视图
- 饼图、柱状图、折线图分析
...
可随账号同步的设置仅限用户偏好和备份偏好，例如主题、交互反馈、预算、分类备注、搜索/导出偏好、同步轮询间隔、WebDAV 配置、iOS 主屏快捷方式配置和自动记录规则。登录态、`authMode`、`lastSyncVersion`、Cloudflare 管理 API 配置、旧版同步凭据、调试和运行状态只保留在本地。
...
Web 端支持安装为 PWA，并缓存静态资源。移动端通过 Capacitor 提供键盘适配、震动反馈、文件导出、分享和 Deep Link 能力。iOS 原生壳会把设置中的快捷方式写入动态 Home Screen Quick Actions；快捷入口使用 `personalledger://add?ledgerId=...&type=...` 打开指定账本的录入页，但不会写入 `lastLedgerId` 或切换正常启动账本；快捷方式编辑弹窗会在输入名称时避让系统键盘。iOS/PWA 界面对按钮、导航、卡片和普通文本禁用 WebKit 长按复制菜单与文本选中，减少长按添加等手势被系统复制行为打断；输入框、文本域和可编辑区域仍可正常编辑。
...
- 买卖本的买入数量、卡密卖出数量和卖出手续费率输入会复用底部小键盘，不唤起系统输入法；底部备注与单价栏采用紧凑布局，右侧按钮可收起或展开小键盘，点击单价、数量或手续费率区域会重新展开并切回对应输入；卡密买入输入槽点击后显示粘贴操作，可从剪贴板粘贴单个密钥或按换行批量粘贴

买卖本录入在同一个页面内切换为买入 / 卖出模式：新建买入和新建卖出都会先进入独立的买卖本类目选择页，不露出后面的账目录入页；选定类目后再进入对应账目录入。买入时输入单价和数量后，系统先计算交易总额 = 单价 × 数量，再按类目的买入手续费率计算最终金额。普通物品类目继续按买入批次记录库存；卡密类目会按买入数量列出同等数量的密钥输入框，保存时把密钥写入 `tradeKeys`，同一类目内不允许重复卡密。卖出时先选择要卖出的类目，类目卡片会显示当前可卖数量；没有可卖库存的卖出类目会置灰并禁止选择。新建买入或新建卖出且尚未选择类目时取消会直接退出录入页，回到点击添加前的界面。普通物品卖出页面只显示仍有剩余的买入批次，用户可以在多个批次上分别填写本次卖出数量；卡密卖出页面输入数量后默认自动按最早未售库存列出对应密钥，也可以通过卖出数量行右侧下拉列表切换为按买入批次选择，并在每个批次上指定本次取出的卡密数量。按批次选择时，卖出数量会按已选卡密数量实时回写，直接修改卖出数量也会按可售顺序自动补齐批次。已列出的卡密会显示对应买入日期、买入记录和单个成本，不展示内部交易编号；卡密卖出页提供单个复制和复制全部按钮，复制全部成功后会弹窗提示“已复制全部卡密”。卖出记录会保存指向买入批次的 `tradeAllocations`；卡密卖出还会保存 `tradeKeyAllocations`，后续同步、CSV 导入导出和利润计算都能保留“卖出哪一批、哪几个密钥”的关系，已卖出的密钥不会再次进入可售列表。卖出手续费率默认使用类目的卖出手续费率，但可以在本次卖出页面单独修改，卖出最终金额为交易总额扣除本次手续费。买卖本首页列表会突出买入数量和卖出数量；卖出记录下方显示卖出金额与利润，买入记录下方显示买入金额和每批买入的已实现卖出利润。买入记录没有对应卖出时利润显示 0，已全部卖出的买入记录会淡化显示。记账本列表仍保持原收入 / 支出金额展示。买卖本首页的利润为当前周期卖出净收入减去对应买入批次的成本，没有卖出时显示 0；旧卖出记录没有 `tradeAllocations` 时继续按同类目历史买入记录的时间顺序匹配成本。
...
- WebDAV 地址、账号、“密码 / 应用密钥”、iOS 主屏快捷方式配置和自动记录规则会随账号设置同步，登录同一账号后可直接使用 WebDAV 备份配置、快捷入口和自动记录。
