...
> 当前版本：7.6.1。本版本优化录入流程：记账本收入/支出与买卖本买入/卖出录入页都支持一键收起或展开底部小键盘；买卖本新建买入和卖出都会先进入类目选择页，选定后再进入账目录入；底部添加按钮点击或长按都会先展开快捷入口。
