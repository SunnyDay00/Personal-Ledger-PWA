...
> 当前版本：7.6.3。本版本新增自动记录：可在设置 > 管理中创建普通记账本的收入 / 支出模板，按每日、每周或每月固定时间自动生成账目，并支持账号云端同步和多设备去重。
...
- 设置 > 管理 > 自动记录可创建普通记账本的收入 / 支出模板；新建时先填写名称、账本和图标，再配置类型、分类、金额和每日 / 每周 / 每月固定时间，输入时弹窗会避开顶部安全区和键盘；App 打开或回到前台时会补齐错过的周期，同一期账目使用稳定 ID 去重，登录同一账号的多设备不会重复生成
- 周 / 月 / 年统计视图
- 饼图、柱状图、折线图分析
